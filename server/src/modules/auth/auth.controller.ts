...
export class AuthController {
	...
	private issueRefreshCookie(
		res: Response,
		result: { accessToken: string; refreshToken: string; user: AuthHttpResponseDto['user'] },
	): AuthHttpResponseDto {
	...
	async register(
		...
		return this.issueRefreshCookie(res, await this.authService.register(registerDto));
	}
	...
	async refresh(
		...
		return this.issueRefreshCookie(res, await this.authService.refreshTokens(userId));
	}
	...
	@ApiOperation({
		summary: 'Logout user',
		description: 'Logs out the user, invalidates the refresh token and clears its cookie',
	})
	@ApiResponse({
		status: 200,
